...
sorted(df.eval_aux({'1': {0, 1}, '2': {0, 2}}))
        ...
        meet = {self.inst.ID}
        #intersection = {self.inst.preds[0].ID} if len(self.inst.preds) > 0 else set()
        intersection = UniversalSet()

        for node in self.inst.preds: 
            # print(intersection)
            # print(env[str(node.ID)])
            intersection = intersection & env[str(node.ID)]
...
sol[3]
    ...
    for inst in insts: 
        equations.append(Dominance_Eq(inst))
